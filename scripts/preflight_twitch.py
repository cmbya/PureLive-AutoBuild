...
def main():
    ...
    print("\n3/6 主播详情")
    shell = gql_persisted(hashes, "ChannelShell", {"login": login})
    user = shell.get("userOrError") or {}
    if not user.get("login") or user.get("stream") is None:
        raise RuntimeError(f"ChannelShell 返回异常：{user}")
    metadata = gql_persisted(
        hashes,
        "StreamMetadata",
        {"channelLogin": login, "includeIsDJ": True},
    )
    if not (metadata.get("user") or {}).get("id"):
        raise RuntimeError("StreamMetadata 用户无效")
    print("✅ 主播详情正常")
